...
-- TODO: Map RHS SITE 2 to the standard EHS_IN schema.
-- Output columns (explicit only):
--   customer_short_code, dc_site_code, asset_id, event_dts (TIMESTAMP), datapoint, metric_value (FLOAT)
-- Guidance:
-- 1) Select only the required columns from {{ ref('interview_model_rhs_SITE2') }} (no SELECT *).
-- 2) Impute / calculate the missing  minute level points from rhs (there are holes in the data) and rationalize your missing value calucation approach
...
select
  *
from {{ ref('interview_model_rhs_SITE2') }}
